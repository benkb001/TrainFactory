...
public class PurchaseClickSystemTest {
    ...
    private static (World, int, Inventory) init() {
        ...
        PurchaseInfo pb = PurchaseInfo.AddItemInfo(productID, 1, cost);
        w.SetComponent<PurchaseButton<PurchaseItem>>(btnEnt, 
            new PurchaseButton<PurchaseItem>((PurchaseItem)pb.Buyable, cost, inv));
        return (w, btnEnt, inv); 
    }
